...
def get_water(g_type):
    ...
    elems = driver.find_elements_by_xpath(
        '//label[@class="el-checkbox custom-checkbox"]'
    )
    for elem in elems:
    ...
    elem = driver.find_element_by_xpath(
        "/html/body/div[3]/div/div/div[1]/div[3]/div[1]/div[2]/div/div[4]/div[1]/span[1]"
    )
    driver.execute_script("arguments[0].click();", elem)
    time.sleep(1.5)
    ...
    for d in des:
        ...
        # 输入站点
        elem = driver.find_element_by_xpath(
            "/html/body/div[3]/div/div/div[1]/div[3]/div[1]/div[2]/div/div[4]/div[2]/input"
        )
        elem.clear()
        # 键入关键词
        elem.send_keys(d)
        ...
        try:
            ...
            while True:
                ...
                if elems[2 + 5 * (count[d] + flag)].text != d:
                    ...
                    driver.execute_script(
                        "arguments[0].click();", elems[2 + 5 * (count[d] + flag)]
                    )
                    break
            time.sleep(5)
            ...
            for step in range(1, 73):
                ...
                css += (
                    "(" + str(step) + ") > td.el-table_1_column_4.el-table__cell > div"
                )
...
def save_exl(title, data_list, hour_list, name, type):
    ...
    path = os.path.join(
        path, str(type) + "_" + name + "_" + hour_list[0][0][0:8] + ".xlsx"
    )
    wb = Workbook()
    ws = wb.active
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        j = i + 1
        if j == 1:
            path = os.path.join(script_dir, "1_qiantang", "location.txt")
        else:
            path = os.path.join(script_dir, "2_datian", "location.txt")
        des = read_txt(path)
        get_water(j)
